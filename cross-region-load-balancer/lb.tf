resource "azurerm_public_ip" "pip-cr-lb" {
  name                = format ("pip-cr-lb-%s", var.regions.region1.location)
  location            = var.regions.region1.location
  resource_group_name = azurerm_resource_group.rg_2["region1"].name
  allocation_method   = "Static"
  sku                 = "Standard"
  sku_tier            = "Global"
  domain_name_label   = "lb-global-example" 
  depends_on          = [azurerm_resource_group.rg_2]
}
